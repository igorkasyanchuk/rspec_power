...
| [🔒 CI Guards](#-ci-guards) | Conditionally run or skip on CI | `:with_ci_only`, `:with_skip_ci` |
...
| [🛢 DB Dump on Failure](#-db-dump-on-failure) | Dump DB tables to CSV when an example fails | `:with_dump_db_on_fail`, `with_dump_db_on_fail: { tables: [...], except: [...] }` |
